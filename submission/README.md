...
submission/
  ...
  fig/
    ...
    ExtendedDataFigures/  — empty; there are no Extended Data figures
```
...
Five main figures (no Extended Data), each built from specific deck slides:
...
| **Fig 4** | `Fig4a_human_scatter.pdf`, `Fig4b_porec_community.pdf`, `Fig4c_mouse_zygotene.pdf` | 20, 21, 22 |
...
`fig:fig5` exist (the mouse meiotic-bouquet result, formerly ED1, is now Fig4c);
do **not** re-introduce `\ref{fig:ed*}`.
...
There are no Extended Data figures: `fig:ed1`…`fig:ed11` were all removed (the
mouse figure became Fig4c). Only `fig:fig1`–`fig:fig5` exist. Do not reference
`fig:ed*`.
