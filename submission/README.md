...
bibtex   paper      # single bibliography
...
Removes `paper.pdf` and all `.aux`, `.bbl`, `.blg`, `.log`, `.out`, `.toc` files. It also removes stale `Meth.*` and `Supp.*` auxiliaries left by older multibib builds.
...
| `paper.blg`  | BibTeX log for the bibliography                   |
...
submission/
  ...
  bibliography.bib        — entries cited in the manuscript
  Makefile                — `make` builds paper.pdf
  compile.sh              — pdflatex -> bibtex -> pdflatex x 2
  BUILD_LOG.md            — compile and validation notes
  README.md               — this file
  fig/
    MainFigures/          — Fig1*…Fig5* (regenerated by scripts/figures/;
                            see scripts/figures/README.md)
    ExtendedDataFigures/  — ED_Fig1_chm13_hic.{pdf,png} (CHM13 Hi-C, replicate of Fig4a)
```
...
Five main figures + one Extended Data figure, each built from specific deck slides:

| Figure | Panels (files in `fig/`)                                                            | Deck slides |
|--------|-------------------------------------------------------------------------------------|-------------|
| **Fig 1** | `Fig1a_genomewide.pdf`, `Fig1b_lengths.pdf`                                      | 3, 4        |
| **Fig 2** | `Fig2a_pggb_layout.png` (top), `Fig2bc_jaccard.pdf` (tree + community, below)    | 6, 10, 12 |
...
| **Fig 4** | `Fig4a_human_scatter.pdf` (HG002) + `Fig4b_porec_community.pdf`; `Fig4c_mouse_zygotene.pdf` | 20, 21, 22 |
| **Fig 5** | `Fig5_pedigree_untangle.pdf`                                                      | 24          |
| **ED 1**  | `ExtendedDataFigures/ED_Fig1_chm13_hic.pdf` (CHM13 Hi-C replicate of Fig4a)      | 21          |

> **ASSET STATUS.** Fig1a, Fig1b, Fig2b/2c, Fig4a, Fig4b, Fig4c and Fig5 are
> regenerated as vector PDFs by `scripts/figures/` (see
> `scripts/figures/README.md`); Fig2a and the Fig3 browser panels are PNGs.
> Re-run the relevant `scripts/figures/make_*.R` to rebuild any figure.
...
`fig:fig5` and `fig:ed1` (CHM13 Hi-C) exist; the mouse meiotic-bouquet result is
Fig4c. Do **not** reference `fig:ed2`+.

## Bibliography

`bibliography.bib` contains the citation database for a single numbered
reference list. The manuscript uses `\cite{...}` throughout the body, Methods,
and legends. Entries in the `.bib` that are no longer cited (e.g. `hao2024snul`,
`bouquet_ChenCEP164Cilia2025`, which were only in dropped Extended Data captions)
are harmlessly ignored by bibtex.
...
Verify you are running `make` / `compile.sh` from inside `submission/`. Both `jnl.cls` and `mathphys.bst` must sit beside `paper.tex`.
...
Only `fig:ed1` (CHM13 Hi-C) exists among Extended Data; `fig:ed2`+ were removed.
`fig:fig1`–`fig:fig5` plus `fig:ed1` are the valid figure refs.
