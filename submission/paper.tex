...
% Hi-C, HG002 Hi-C, HG002 CiFi), all 50 kbp all-points. ED2 = maternal
% whole-genome pedigree recombination companion scans for Fig 5. The mouse
...
(descriptive pointwise Spearman $\rho = 0.419$, 0.614, 0.576 and 0.245 at
...
These segments are modest in length (20--40 kb for the autosomal calls, and
...
visible in the same display framework (Extended Data Fig.~\ref{fig:ed2}).
...
\begin{figure}[h!]
  ...
  \caption{\textbf{Genome-wide subtelomeric sharing and PHR extent.}
    ...
    (232 HPRC v2 individuals and CHM13). Each chromosome is shown in 100 kb windows coloured by the number of
...
\begin{figure}[h!]
  ...
  \caption{\textbf{Sequence similarity tracks three-dimensional proximity.}
    ...
    the PHRs' exact coordinates, summed over the 50 kbp bin-pairs the
...
\begin{figure}[h!]
  ...
  \caption{\textbf{Whole-genome pedigree view of putative subtelomeric exchange in a paternal transmission.}
    PAN027 inherited the paternal query haplotype shown in the middle track from
    PAN011; the upper and lower tracks show the two PAN011 father haplotypes.
    Chromosomes are concatenated in native chromosome order and scaled by their
    assembled lengths; thin black bars mark chromosome boundaries. Light-gray
    ribbons show same-chromosome father-child homologous chains ($\geq 10$ kb,
    identity $\geq 0.95$). Colored ribbons show 2 kb query-space windows,
    grouped into $\geq 10$ kb runs, where the best non-homologous PAN011 match
    beats the best same-chromosome or homologous match in the same
    IMPG scan. The male Xp/Yp PAR1 signal is an internal positive control for
    obligate sex-chromosome recombination. The chr5q/chr1p and chr9q/chr3q
    ribbons are putative autosomal PHR-exchange candidates; PHR and community
    labels were assigned only after calling, and the highlighted pairs fall in
    the expected population arm-community/Jaccard groups (Xp/Yp C15,
    chr5q/chr1p C11, chr9q/chr3q C3). Acrocentric p-arm matches are also
    recovered, as expected from known acrocentric homology.}
  \label{fig:fig5}
\end{figure}
...
genome, all at 50 kbp via \texttt{sequence\_hic\_correlation.py}.
The primary comparison is HG002 Pore-C (Fig.~\ref{fig:fig4}A; $\rho = 0.381$,
$n = 2{,}830$); the replicates in Extended Data Fig.~\ref{fig:ed1} are CHM13 Hi-C
($\rho = 0.716$, $n = 652$), HG002 Hi-C ($\rho = 0.662$, $n = 2{,}544$) and HG002
CiFi ($\rho = 0.191$, $n = 2{,}757$; the sparsest assay, hence the weakest signal).
...
0.245 respectively, all $n = 1{,}135$; Fig.~\ref{fig:fig4}C); at coarser 50 kb
resolution the per-stage correlations flatten ($\rho = 0.37$--0.43).
These are reported as descriptive point estimates without a formal
stage-contrast test.
...
For Fig.~\ref{fig:fig5}, the PAN027 paternal haplotype was used as query against
...
For Extended Data Fig.~\ref{fig:ed2}, the same direct-alignment workflow,
...
figure-generation inputs for Fig.~\ref{fig:fig5} and Extended Data
Fig.~\ref{fig:ed2}.
...
\begin{figure}[p!]
  \centering
  \textbf{A}\par
  \includegraphics[width=\linewidth]{fig/ExtendedDataFigures/ED_Fig2a_PAN027_maternal_recombination.png}
  \vspace{0.75em}

  \textbf{B}\par
  \includegraphics[width=\linewidth]{fig/ExtendedDataFigures/ED_Fig2b_PAN028_maternal_recombination.png}
  \caption{\textbf{Maternal whole-genome homologous-competition scans companion
    to Fig.~\ref{fig:fig5}.} The same direct-alignment and IMPG class-winner
    procedure used for the paternal Fig.~\ref{fig:fig5} scan was applied to two
    maternal transmissions. In each panel, the middle track is the child query
    haplotype and the upper and lower tracks are the two maternal donor
    haplotypes, with chromosomes concatenated in native order and scaled by
    assembled length. Light-gray ribbons show same-chromosome maternal
    homologous chains ($\geq 10$ kb, identity $\geq 0.95$). Colored ribbons show
    grouped 2 kb query windows in which the best non-homologous maternal match
    beats the best same-chromosome homologous match. Before the IMPG similarity
    step the raw SweepGA/FastGA f32 PAF was filtered with \texttt{sweepga
    --num-mappings 10:10 --scaffold-jump 0 --scoring ani}.
    \textbf{(A)} PAN027 maternal haplotype compared with the joint PAN010 mother
    target. The drawn high-confidence non-homologous runs are
    acrocentric-acrocentric. \textbf{(B)} PAN028 maternal haplotype compared
    with the joint PAN027 mother target. The display is dominated by
    acrocentric-acrocentric runs, with one chr5/chr1 candidate and one
    acrocentric-other run. These panels provide maternal-transmission context
    for the Fig.~\ref{fig:fig5} paternal candidate calls rather than additional
    breakpoint-resolved event calls.}
  \label{fig:ed2}
\end{figure}
...
homologous-competition scan and the companion maternal scans in Extended Data
Fig.~\ref{fig:ed2}. Rows include the PAR1 positive-control runs, the paternal
