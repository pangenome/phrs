...
The model predicts that arms sharing a sequence community should show more
three-dimensional contact, with the strongest relationship expected at the
bouquet stage
(Fig.~\ref{fig:fig4}).
Mouse provides the stage-resolved test: the same B6 and CAST T2T assemblies, the
same PHR sequence identities and the same read-placement procedure are used at
each meiotic stage \cite{Francis2025,Zuo2021}.
A signal introduced solely by read placement should therefore be stage-invariant.
Instead, contact between sequence-similar PHRs rises from leptotene into
zygotene and then falls (per-PHR-pair Spearman $\rho = 0.419$, $0.614$, $0.576$
and $0.245$; Fig.~\ref{fig:fig4}C), matching the stage at which telomeres
cluster in the bouquet.
In human contact-map data, contact also increases with PHR similarity.
We measure this at the finest resolution, per individual PHR pair within a
single genome and with no community assignment or cross-sample averaging, in
HG002 Pore-C (Fig.~\ref{fig:fig4}A; pointwise Spearman $\rho = 0.38$), with the
same relationship in a second genome and two further assays (CHM13 Hi-C
$\rho = 0.72$, HG002 Hi-C $\rho = 0.66$, HG002 CiFi $\rho = 0.19$; all
$p < 10^{-20}$; Extended Data Fig.~\ref{fig:ed1}).
Because some non-homologous human PHR tracts are sequence-identical, no
mapping-quality threshold can assign every read in those tracts to its true
chromosome end.
We therefore read the human maps as concordant evidence that sequence-similar
subtelomeres contact one another more often, while the stage-resolved mouse data
provide the timing test against a constant mapping confound.
Ordering a Pore-C contact matrix by sequence community shows the same community
block structure (Fig.~\ref{fig:fig4}B) \cite{Ulahannan2019}.
...
centromere-ward of each PHR argues against a simple multi-mapping artefact.
...
\begin{figure}[h!]
  ...
  \caption{\textbf{Pangenome similarity structure and communities.}
    ...
    partition, with community colour bands. The blocks recover every known case of
...
\begin{figure}[h!]
  ...
  \caption{\textbf{Sequence similarity tracks three-dimensional proximity.}
    \textbf{(A)} Sequence-similar subtelomeres contact one another more often in
    three dimensions (HG002 Pore-C). Each dot is one inter-chromosomal pair of
    ...
    $p = 1.2 \times 10^{-118}$), the analogous per-pair, no-averaging test as
    panel (A). Right,
...
Support was estimated by a 1,000-replicate character-level bootstrap: each
replicate resampled PHR flanks with replacement, rebuilt the arm-level distance
matrix from per-PHR cross-chromosome involvement, and recomputed NJ and UPGMA
trees. The implementation is included in the public analysis-code repository.
...
validated method: the flanking unique-sequence control argues against broad
multi-mapping-driven inflation but does not bound a possible bias from uniform
MAPQ0 distribution across
...
stronger than PHR B/W, arguing against multi-mapping-driven inflation as the sole
explanation.
A strict-MAPQ re-binning from upstream valid-pair files (both mates MAPQ
$\geq 30$) reproduced the flanking B/W within Poisson noise while PHR-internal
contact collapsed to a noise floor; observed-over-expected normalisation handles the rare
...
\cite{hic3d_dixon2012,hic3d_imakaev2012,Ulahannan2019,hic3d_cifi2025}.
...
patch geometry and parent/child haplotype consistency
