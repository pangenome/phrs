...
adjacent flanks, which lack the shared sequence used to define the pairs. A whole-genome comparison in
a three-generation pedigree assembled end to end recovers the known Xp/Yp PAR1
exchange and identifies putative non-homologous subtelomeric exchange patches
...
From 232 HPRC v2 individuals we used both haplotype assemblies each together
...
\begin{figure}[h!]
  ...
  \caption{\textbf{Genome-wide subtelomeric sharing and PHR extent.}
    ...
    high-identity blocks mark inter-chromosomal sharing at chromosome ends, and the
    acrocentrics and PARs appear as positive controls.
    \textbf{(B)} PHR length distribution by chromosome end, p-arms and q-arms
    separately, colored by the number of PHRs per 10 kb length bin
    (pseudo-log scale) across the terminal 500 kb, with length measured from the
...
\begin{figure}[h!]
  ...
  \caption{\textbf{Whole-genome pedigree view of putative subtelomeric exchange in a paternal transmission.}
    ...
    grouped into $\geq 10$ kb runs, where the best PAN011 non-homologous father
    match beats the best same-chromosome or homologous father match in the same
...
The arm-flank census uses 465 near-complete assemblies: two haplotype
assemblies for each of the 232 HPRC v2 individuals plus the haploid CHM13 anchor.
...
\begin{figure}[p!]
  ...
  \caption{\textbf{Maternal whole-genome homologous-competition scans companion
    ...
    beats the best same-chromosome homologous match after the required
    SweepGA/FastGA f32 raw PAF was reduced by \texttt{sweepga --num-mappings
    10:10 --scaffold-jump 0 --scoring ani} before IMPG similarity.
