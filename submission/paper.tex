...
This male transmission provides an internal positive control: the same procedure
recovers PAR1 sharing between Xp and Yp, the obligate site of male
sex-chromosome recombination.
It also recovers two putative autosomal PHR exchanges, chr5q/chr1p and
chr9q/chr3q, in which the best inter-chromosomal father match beats the
same-chromosome or homologous father match across adjacent windows.
...
The two candidates differ in support. An independent aligner (wfmash)
...
a weaker, aligner-dependent candidate.
We interpret both as recombination-compatible PHR-exchange candidates rather
than breakpoint-resolved or event-level truth calls, and confirming individual
WashU events will require orthogonal long-read recombination maps
...
\begin{figure}[h!]
  ...
  \caption{\textbf{Pedigree-resolved donor-recipient ribbons for candidate PHR exchange.}
    PAN027 inherited a paternal haplotype from PAN011; the child query haplotype
    was compared with a joint two-haplotype PAN011 father target. Each panel
    shows one 500 kb subtelomeric PAN027 recipient window above the PAN011 donor
    window(s) selected by the best inter-chromosomal father match. Colored blocks
    mark 2 kb query-space windows whose inter-chromosomal father match beats the
    same-chromosome or homologous father match, and ribbons connect the
    recipient window to the corresponding donor interval. Adjacent 2 kb windows
    are merged for display when donor chromosome, haplotype and sequence
    coordinates are consistent and nearby. Black bars mark PHR/PAR intervals; in
    the Xp/Yp panel, PAR1 is drawn across the full visible 500 kb p-tip window
    because the canonical PAR1 interval extends beyond the zoom. Gray triangles
    indicate that the chromosome continues outside the plotted 500 kb window.
    \textbf{(A)} Male Xp/Yp transmission, including PAR1 as an internal positive
    control for obligate male sex-chromosome recombination. \textbf{(B,C)} Two
    putative recombination-compatible autosomal PHR exchanges, chr5q/chr1p and
    chr9q/chr3q. PHR/community labels were assigned only after calling; the
    highlighted pairs are supported by the population arm-community/Jaccard
    analysis (Xp/Yp C15, chr5q/chr1p C11, chr9q/chr3q C3). Of the two autosomal
    candidates, only chr9q/chr3q is reproduced by an independent aligner
    (wfmash); chr5q/chr1p is a weaker, aligner-dependent candidate (main text).}
  \label{fig:fig5}
\end{figure}
...
to 1:1, 4:4 and 10:10 mapping classes; the main panels use the 10:10 class.
The query was summarized in 2 kb windows. We excluded centromeric windows,
windows without alignment support and windows with excessive inter-chromosomal
PAF depth (maximum depth 100).
For each retained window, we ran \texttt{impg similarity} 0.4.1 with POA, no
merge, many:many mapping and scaffold jump 0. A window was assigned to an
inter-chromosomal class when its best PAN011 father match on a non-homologous
chromosome beat the same-chromosome or homologous father match. Adjacent 2 kb
windows were merged only for display when donor chromosome, donor haplotype,
donor sequence and nearby recipient and donor coordinates were consistent. PHR
and arm-community labels were assigned after calling and were not used to select
the windows.
