...
// ══════════════════════════════════════════════════════════════════════════════
// PAGE 18 — Slide 15: Closer (text-only)
// ══════════════════════════════════════════════════════════════════════════════

#text-slide(
  "15",
  "Thesis closer — text only",
  ...
  [
    ...
    *Method (slide 3):* Implicit pangenome graph: wfmash all-vs-all over 18,827 telomere-anchored flanks, ~12% of all pairs evaluated — *230× above the Erdős-Rényi connectivity threshold* (p\* = log(n)/n ≈ 5.21×10⁻⁴). No chromosome partitioning, no GFA.

    *Empirical (slides 4–9):* 15,668 PHRs across 41/48 arms, median 105 kb / mean 144 kb — PAR2-scale pseudohomology at nearly every chromosome end. Named clades: Xp/Yp & Xq/Yq via PARs, *acrocentric short arms* (C7), *10p–18p*, the big q-arm clade (*22q–21q–19q–1q–13q–17q*), and *4q–10q DUX4*.

    *Mechanism (slides 10–12):* Hi-C/Pore-C/Dip-C/sperm all recover community-structured 3D contacts (Mantel ρ = 0.296, p = 0.002; per-pair ρ = 0.674 in CHM13 Hi-C). Median PHR (105 kb) sits at the base of a single meiotic loop — the *bouquet* is the predicted exchange venue.

    *Proof (slide 13):* WashU T2T pedigree: *494/538 (92%)* inter-chromosomal patches fall inside Leiden communities — 133 gene-conversion-like, 16 crossover-like.

    *Biology (slide 14):* D4Z4/DUX4 (4q↔10q) is the disease-revealed instance of the same process.

    #block(
      fill: col-why-bg,
      inset: (x: 0.15in, y: 0.12in),
      radius: 4pt,
      stroke: (left: 4pt + col-why-bar),
      width: 100%,
    )[
      #text(size: 13pt, weight: "bold")[Thesis: ] #text(size: 13pt)[_Subtelomeres concertedly evolve through ongoing inter-chromosomal exchange — observable in pedigrees, predicted by 3D, recovered by an implicit pangenome graph across HPRC v2._]
    ]
  ],
  [The closing slide recapitulates all five pillars (method → empirical → mechanism → proof → biology) in 70 seconds and ends on the verbatim thesis sentence — the title's "concerted evolution" echoed from slide 01 to slide 15.],
)
