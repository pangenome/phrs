...
with open('gprofiler_results.json', 'r') as f:
    data = json.load(f)

print("=== g:Profiler GO Enrichment Analysis Results ===")
...
print(f"Query info: {len(query_genes)} genes analyzed")
...
if go_bp_results:
    ...
    with open('phr_GO_BP_enrichment.csv', 'w', newline='') as csvfile:
...
if go_mf_results:
    ...
    with open('phr_GO_MF_enrichment.csv', 'w', newline='') as csvfile:
        fieldnames = ['term_id', 'term_name', 'p_value', 'adjusted_p_value', 'term_size', 'query_size', 'intersection_size', 'effective_domain_size', 'source']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()
...
if kegg_results:
    ...
    with open('phr_KEGG_enrichment.csv', 'w', newline='') as csvfile:
        fieldnames = ['term_id', 'term_name', 'p_value', 'adjusted_p_value', 'term_size', 'query_size', 'intersection_size', 'effective_domain_size', 'source']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()
...
print(f"\nGene mapping info:")
query_genes = meta['query_metadata']['queries']['query_1']
print(f"Total genes submitted: 245")
print(f"Genes recognized: {len(query_genes)}")
print(f"Genes lost: {245 - len(query_genes)}")

print(f"\nResults saved:")
print(f"- phr_GO_BP_enrichment.csv ({len(go_bp_results)} terms)")
print(f"- phr_GO_MF_enrichment.csv ({len(go_mf_results)} terms)")
print(f"- phr_KEGG_enrichment.csv ({len(kegg_results)} pathways)")