...
The relationship persists at the finest resolution, per individual PHR pair
within a single genome and with no community assignment or cross-sample
averaging (Extended Data Fig.~\ref{fig:ed2}; pointwise Spearman $\rho = 0.38$
for HG002 Pore-C and $0.72$ for CHM13 Hi-C).
...
\begin{figure}[h!]
  ...
  \caption{\textbf{Sequence similarity predicts three-dimensional proximity.}
    ...
    contact. Each dot is one inter-chromosomal chromosome-arm pair; \emph{x},
    mean subtelomeric sequence similarity (PHR Jaccard); \emph{y}, 3D contact
    frequency at 50 kbp (log scale, with zero-contact pairs placed at the axis
    floor). Contact rises with similarity in both HG002 Pore-C (pointwise
    Spearman $\rho = 0.485$, $n = 803$, $p = 1.6 \times 10^{-48}$) and CHM13
    Hi-C ($\rho = 0.674$, $n = 688$, $p = 3.0 \times 10^{-92}$); line, linear
    fit.
...
Pore-C $\rho = 0.485$, $n = 803$; CHM13 Hi-C $\rho = 0.674$, $n = 688$) use the
population mean Jaccard (averaged over all PHR sequence-pair comparisons between
the two arms across the 233 samples); the community-ordered Pore-C matrix of
Fig.~\ref{fig:fig4}b ($B/W = 0.056$) is at 50 kb.
The community-free, single-sample companion (Extended Data Fig.~\ref{fig:ed2})
instead takes each inter-chromosomal PHR sequence pair as a point, correlating
its pangenome-graph Jaccard with its length-normalised Hi-C/Pore-C contact at
the pair's exact coordinates within one genome (HG002 Pore-C $\rho = 0.381$,
$n = 2830$; CHM13 Hi-C $\rho = 0.716$, $n = 652$; \texttt{sequence\_hic\_correlation.py}).
...
\begin{figure}[htb!]
  \centering
  \includegraphics[width=\linewidth]{fig/ExtendedDataFigures/ED_Fig2_allpoints.png}
  \caption{\textbf{Per-PHR-pair sequence similarity versus three-dimensional
    contact (single-sample, community-free).}
    A finer-grained, assumption-free companion to Fig.~\ref{fig:fig4}a: instead
    of one point per chromosome-arm pair, each point is one inter-chromosomal
    pair of pseudohomologous regions (PHRs) within a single genome, so both axes
    derive from the same sample. \emph{x}, Jaccard sequence similarity of the two
    PHRs from the pangenome graph (0 = no shared sequence, 1 = identical);
    \emph{y}, their balanced Hi-C/Pore-C contact at the PHRs' exact coordinates,
    binned at 50~kbp and normalized by interval length so longer PHRs are not
    counted more (log scale; pairs with zero contact placed at the floor).
    \textbf{(left)} HG002 Pore-C ($n = 2{,}830$ PHR pairs); \textbf{(right)}
    CHM13 Hi-C ($n = 652$). Higher PHR similarity is associated with more contact
    in both (pointwise Spearman $\rho = 0.381$, $p = 1.2 \times 10^{-98}$ and
    $\rho = 0.716$, $p = 1.2 \times 10^{-103}$; line, linear fit). HG002
    contributes more pairs than CHM13 because it is diploid (77 PHR sequences
    across both haplotypes) whereas CHM13 is haploid (37). This community-free
    test recovers the Fig.~\ref{fig:fig4}a relationship without any community
    assignment or cross-sample averaging.}
  \label{fig:ed2}
\end{figure}
