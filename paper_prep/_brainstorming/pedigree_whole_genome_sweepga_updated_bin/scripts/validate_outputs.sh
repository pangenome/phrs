...
python3 "$SCRIPT_DIR/candidate_window_support.py"
...
for cid in PAN027pat_vs_PAN011_joint PAN027mat_vs_PAN010_joint PAN028mat_vs_PAN027_joint; do
    ...
    chop_dir="$PACKAGE_DIR/chopped_paf_l${PAF_CHOP_LENGTH:-10000}_o${PAF_CHOP_OVERLAP:-0}"
    test -s "$chop_dir/${cid}.chopped_l${PAF_CHOP_LENGTH:-10000}_o${PAF_CHOP_OVERLAP:-0}.paf.gz"
...
test -s "$PACKAGE_DIR/summaries/chop_manifest_l${PAF_CHOP_LENGTH:-10000}_o${PAF_CHOP_OVERLAP:-0}.tsv"
test -s "$PACKAGE_DIR/summaries/filter_manifest.tsv"
test -s "$PACKAGE_DIR/summaries/paf_file_summary.tsv"
test -s "$PACKAGE_DIR/summaries/candidate_window_support.tsv"
