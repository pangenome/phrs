...
tail -n +2 "$COMPARISONS" | while IFS=$'\t' read -r comparison_id _rest; do
    ...
    stdout="$PACKAGE_DIR/logs/${comparison_id}.frequency100_many_many_j0.%j.out"
    job_id="$(
        sbatch --parsable \
            --job-name="sg-f100-${comparison_id}" \
            ...
            "$SCRIPT_DIR/run_sweepga_frequency_one.sh" "$comparison_id" 100 many:many 0
    )"
    printf 'raw_frequency100\t%s\t%s\tfrequency100\t100\tmany:many\t0\t/dev/shm\t%s\n' "$comparison_id" "$job_id" "$stdout" >> "$JOBS"
done
