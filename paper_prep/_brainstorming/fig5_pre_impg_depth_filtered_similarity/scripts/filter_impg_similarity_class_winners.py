...
def row_score(row: dict[str, str] | None) -> float | None:
    if row is None:
        return None
...
def emit_group(
    ...
    class_counts = {"homolog": 0, "interchrom": 0}
    for row in rows:
        ...
        winner_class = "homolog" if query_chrom == other_chrom else "interchrom"
    ...
    homolog = best.get("homolog")
    interchrom = best.get("interchrom")
    homolog_row = homolog[1] if homolog else None
    interchrom_row = interchrom[1] if interchrom else None
    homolog_score = row_score(homolog_row)
    interchrom_score = row_score(interchrom_row)
    delta = None
    if homolog_score is not None and interchrom_score is not None:
        delta = interchrom_score - homolog_score
    if interchrom_score is not None and (homolog_score is None or interchrom_score > homolog_score):
        winner_class = "interchrom"
    elif homolog_score is not None and (interchrom_score is None or homolog_score > interchrom_score):
        winner_class = "homolog"
    elif homolog_score is not None:
        winner_class = "tie"
    else:
        winner_class = "none"
    ...
    out = {
        ...
        "start": first["start"],
        "end": first["end"],
        "query_chrom": chrom_name(first["chrom"]),
        "raw_candidate_count": raw_count,
        "homolog_candidate_count": class_counts["homolog"],
        "interchrom_candidate_count": class_counts["interchrom"],
        "winner_class": winner_class,
        "homolog_score": fmt_number(homolog_score),
        "interchrom_score": fmt_number(interchrom_score),
        "delta_interchrom_minus_homolog": fmt_number(delta),
    }

    for cls, record in (("homolog", homolog), ("interchrom", interchrom)):
        if record is None:
            for field in ("query_seq", "other_seq", "other_chrom", *source_fields):
                out[f"{cls}_{field}"] = ""
            continue
        _rr, row, query_seq, other_seq = record
        out[f"{cls}_query_seq"] = query_seq
        out[f"{cls}_other_seq"] = other_seq
        out[f"{cls}_other_chrom"] = chrom_name(other_seq)
        for field in source_fields:
            out[f"{cls}_{field}"] = row[field]
...
def main() -> None:
    ...
    with args.skip_report.open("w", newline="") as skip_handle:
        ...
        prefix_fields = [
            ...
            "homolog_score",
            "interchrom_score",
            "delta_interchrom_minus_homolog",
        ]
        source_fields = reader.fieldnames
        class_fields = []
        for cls in ("homolog", "interchrom"):
            class_fields.extend([f"{cls}_query_seq", f"{cls}_other_seq", f"{cls}_other_chrom"])
            class_fields.extend(f"{cls}_{field}" for field in source_fields)
        writer = csv.DictWriter(sys.stdout, delimiter="\t", fieldnames=prefix_fields + class_fields, lineterminator="\n")
        writer.writeheader()
        ...
        for row in reader:
            ...
            if current_key is not None and key != current_key:
                emit_group(writer, skip_writer, group_rows, source_fields, args.max_candidates)
                group_rows = []
            current_key = key
