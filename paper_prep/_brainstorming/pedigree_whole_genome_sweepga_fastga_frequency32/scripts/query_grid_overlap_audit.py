...
F16_AUDIT = os.path.abspath(
    os.path.join(
        PACKAGE_DIR,
        "..",
        "pedigree_whole_genome_sweepga_fastga_frequency16",
        "summaries",
        "query_grid_overlap_audit.tsv",
    )
)
...
def summarize_filtered_candidate(path, candidate):
    empty = {
        ...
        "target_groups": "",
        "target_adjacent_switches": 0,
        "target_overlap_switch_pairs": 0,
        "chr3_rows": 0,
    ...
    target_adjacent_switches = 0
    target_overlap_switches = 0
    for prev, cur in zip(rows_sorted, rows_sorted[1:]):
        if prev[2] != cur[2]:
            target_adjacent_switches += 1
            if cur[0] < prev[1]:
                target_overlap_switches += 1
    ...
    return {
        ...
        "target_adjacent_switches": target_adjacent_switches,
        "target_overlap_switch_pairs": target_overlap_switches,
...
def read_f16_baseline():
    baseline = {}
    if not os.path.exists(F16_AUDIT):
        return baseline
    for row in read_tsv(F16_AUDIT):
        if row.get("artifact_status") != "OK":
            continue
        if not row.get("event_id", "").endswith("_PHR_candidate"):
            continue
        key = (row["event_id"], row["comparison_id"], int(row["chop_length_bp"]))
        baseline[key] = row
    return baseline
...
def delta_text(current, baseline, field):
    base = int_field(baseline, field) if baseline else None
    if base is None:
        return "", ""
    delta = int(current[field]) - base
    return str(base), str(delta)


def support_direction(current_chr3_union, baseline):
    if not baseline:
        return "no_f16_baseline"
    base = int_field(baseline, "chr3_query_union_bp")
    if base is None:
        return "no_f16_baseline"
    if current_chr3_union > base:
        return "more_chr3_support_than_f16"
    if current_chr3_union < base:
        return "less_chr3_support_than_f16"
    return "same_chr3_support_as_f16"
...
def main():
    ...
    f16_baseline = read_f16_baseline()
    rows = []
    examples = []
    completed_keys = set()
    for manifest in manifest_rows:
        ...
        f16 = f16_baseline.get((event_id, comparison_id, length)) if candidate else None
        f16_values = {}
        for field in current_for_delta:
            base, delta = delta_text(current_for_delta, f16, field)
            f16_values[f"f16_{field}"] = base
            f16_values[f"f32_minus_f16_{field}"] = delta
        ...
        rows.append({
            ...
            "target_adjacent_switches": str(filtered["target_adjacent_switches"]),
            "target_overlap_switch_pairs": str(filtered["target_overlap_switch_pairs"]),
            "chr3_retained_rows": str(filtered["chr3_rows"]),
            "chr3_sum_overlap_bp": str(filtered["chr3_sum_overlap_bp"]),
            "chr3_query_union_bp": str(filtered["chr3_query_union_bp"]),
            "chr3_query_redundant_bp": str(filtered["chr3_query_redundant_bp"]),
            ...
            "f16_post_filter_rows_in_window": f16_values.get("f16_post_filter_rows_in_window", ""),
            "f32_minus_f16_post_filter_rows_in_window": f16_values.get("f32_minus_f16_post_filter_rows_in_window", ""),
            "f16_sum_overlap_bp": f16_values.get("f16_sum_overlap_bp", ""),
            "f32_minus_f16_sum_overlap_bp": f16_values.get("f32_minus_f16_sum_overlap_bp", ""),
            "f16_query_union_bp": f16_values.get("f16_query_union_bp", ""),
            "f32_minus_f16_query_union_bp": f16_values.get("f32_minus_f16_query_union_bp", ""),
            "f16_query_redundant_bp": f16_values.get("f16_query_redundant_bp", ""),
            "f32_minus_f16_query_redundant_bp": f16_values.get("f32_minus_f16_query_redundant_bp", ""),
            "f16_chr3_retained_rows": f16_values.get("f16_chr3_retained_rows", ""),
            "f32_minus_f16_chr3_retained_rows": f16_values.get("f32_minus_f16_chr3_retained_rows", ""),
            "f16_chr3_sum_overlap_bp": f16_values.get("f16_chr3_sum_overlap_bp", ""),
            "f32_minus_f16_chr3_sum_overlap_bp": f16_values.get("f32_minus_f16_chr3_sum_overlap_bp", ""),
            "f16_chr3_query_union_bp": f16_values.get("f16_chr3_query_union_bp", ""),
            "f32_minus_f16_chr3_query_union_bp": f16_values.get("f32_minus_f16_chr3_query_union_bp", ""),
            "f16_chr3_query_redundant_bp": f16_values.get("f16_chr3_query_redundant_bp", ""),
            "f32_minus_f16_chr3_query_redundant_bp": f16_values.get("f32_minus_f16_chr3_query_redundant_bp", ""),
            "f16_chr3_haplotype_counts": f16["chr3_haplotype_counts"] if f16 else "",
            "f16_chr3_retained_chunks": f16["chr3_retained_chunks"] if f16 else "",
            "f32_vs_f16_chr3_support": support_direction(filtered["chr3_query_union_bp"], f16) if candidate else "",
    ...
    for status_name in sorted(os.listdir(STATUS_DIR)):
        ...
        for status in read_tsv(os.path.join(STATUS_DIR, status_name)):
            ...
            rows.append({
                ...
                "target_adjacent_switches": "",
                "target_overlap_switch_pairs": "",
                "chr3_retained_rows": "",
                "chr3_sum_overlap_bp": "",
                "chr3_query_union_bp": "",
                ...
                "f16_post_filter_rows_in_window": "",
                "f32_minus_f16_post_filter_rows_in_window": "",
                "f16_sum_overlap_bp": "",
                "f32_minus_f16_sum_overlap_bp": "",
                "f16_query_union_bp": "",
                "f32_minus_f16_query_union_bp": "",
                "f16_query_redundant_bp": "",
                "f32_minus_f16_query_redundant_bp": "",
                "f16_chr3_retained_rows": "",
                "f32_minus_f16_chr3_retained_rows": "",
                "f16_chr3_sum_overlap_bp": "",
                "f32_minus_f16_chr3_sum_overlap_bp": "",
                "f16_chr3_query_union_bp": "",
                "f32_minus_f16_chr3_query_union_bp": "",
                "f16_chr3_query_redundant_bp": "",
                "f32_minus_f16_chr3_query_redundant_bp": "",
                "f16_chr3_haplotype_counts": "",
                "f16_chr3_retained_chunks": "",
                "f32_vs_f16_chr3_support": "",
    ...
    fields = [
        ...
        "target_chrom_haplotype_metrics", "target_adjacent_switches",
        "target_overlap_switch_pairs", "chr3_retained_rows", "chr3_sum_overlap_bp",
        ...
        "row_start_chr3_overlap_hap_switch_pairs",
        "f16_post_filter_rows_in_window", "f32_minus_f16_post_filter_rows_in_window",
        "f16_sum_overlap_bp", "f32_minus_f16_sum_overlap_bp",
        "f16_query_union_bp", "f32_minus_f16_query_union_bp",
        "f16_query_redundant_bp", "f32_minus_f16_query_redundant_bp",
        "f16_chr3_retained_rows", "f32_minus_f16_chr3_retained_rows",
        "f16_chr3_sum_overlap_bp", "f32_minus_f16_chr3_sum_overlap_bp",
        "f16_chr3_query_union_bp", "f32_minus_f16_chr3_query_union_bp",
        "f16_chr3_query_redundant_bp", "f32_minus_f16_chr3_query_redundant_bp",
        "f16_chr3_haplotype_counts", "f16_chr3_retained_chunks",
        "f32_vs_f16_chr3_support", "audit_conclusion",
    ]
