...
tail -n +2 "$COMPARISONS" | while IFS=$'\t' read -r comparison_id _rest; do
    ...
    stdout="$PACKAGE_DIR/logs/${comparison_id}.frequency32_many_many_j0.%j.out"
    job_id="$(
        sbatch --parsable \
            --job-name="sg-f32-${comparison_id}" \
            ...
            "$SCRIPT_DIR/run_sweepga_frequency_one.sh" "$comparison_id" 32 many:many 0
    )"
    printf 'raw_frequency32\t%s\t%s\tfrequency32\t32\tmany:many\t0\t/dev/shm\t%s\n' "$comparison_id" "$job_id" "$stdout" >> "$JOBS"
done
