...
def main():
        ...
        if matched:
            for path in matched:
                for row in summarize_window(path, window):
                    row["layer"] = "raw_frequency32"
                    row["raw_paf_status"] = "OK"
                    row["note"] = "Raw PAF produced by explicit --fastga-frequency 32 many:many scaffold-jump 0 run."
                    rows.append(row)
        else:
            rows.append({
                "candidate_id": window["candidate_id"],
                "comparison_id": window["comparison_id"],
                "layer": "raw_frequency32",
                "paf_path": "none",
    ...
    if pathological:
        ...
        for row in rows:
            ...
            row["note"] = (
                "No raw PAF was emitted before the frequency-32 Slurm job was cancelled; "
