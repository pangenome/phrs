...
printf "comparison_id\tjob_id\tnum_mappings\tscaffold_jump\tstdout\n" > "$STATUS"
...
tail -n +2 "$COMPARISONS" | cut -f1 | while read -r cid; do
    ...
    submit_out="$(sbatch --parsable \
        ...
        --job-name="dsga_${cid}" \
        --cpus-per-task="${SWEEPGA_CPUS:-16}" \
        --mem="${SWEEPGA_MEM:-64G}" \
        --time="${SWEEPGA_TIME:-24:00:00}" \
        --output="$stdout" \
        "$SCRIPT_DIR/run_sweepga_one.sh" "$cid" many:many 0)"
    job_id="${submit_out%%;*}"
    printf "%s\t%s\tmany:many\t0\t%s\n" "$cid" "$job_id" "$stdout" >> "$STATUS"
done
