...
WFMASH_BIN="${WFMASH_BIN:-$PACKAGE_DIR/tools/wfmash-v0.24.2-built/wfmash}"
WFMASH_RUN_LABEL="${WFMASH_RUN_LABEL:-current_v0.24.2}"
...
tail -n +2 "$COMPARISONS" | cut -f1 | while read -r cid; do
    ...
    tail -n +2 "$PARAMETERS" | cut -f1,3 | while IFS=$'\t' read -r parameter_set opts; do
        ...
        out_paf="$PACKAGE_DIR/raw_paf/${WFMASH_RUN_LABEL}/${cid}.${parameter_set}.wfmash-v0.24.2.paf.gz"
        submit_out="$(sbatch --parsable \
            --export=ALL,PACKAGE_DIR="$PACKAGE_DIR",WFMASH_SCRATCH_BASE="$SCRATCH_BASE",WFMASH_BIN="$WFMASH_BIN",WFMASH_RUN_LABEL="$WFMASH_RUN_LABEL" \
