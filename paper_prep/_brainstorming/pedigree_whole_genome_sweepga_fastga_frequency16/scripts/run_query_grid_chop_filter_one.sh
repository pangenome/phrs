...
chop_command="$PIGZ -dc $raw | $PAFCHOP_BIN --length $chop_length --overlap 0 --chunk-mode query-grid --comparison-id $comparison_id --summary $tmp_summary | $PIGZ $PIGZ_COMPRESSION_LEVEL -p $THREADS > $tmp_chopped"
...
exec > >(tee "$log") 2>&1
{
    ...
    [[ -s "$raw" ]]
    "$PIGZ" -dc "$raw" \
        | "$PAFCHOP_BIN" --length "$chop_length" --overlap 0 --chunk-mode "$CHOP_MODE" --comparison-id "$comparison_id" --summary "$tmp_summary" \
