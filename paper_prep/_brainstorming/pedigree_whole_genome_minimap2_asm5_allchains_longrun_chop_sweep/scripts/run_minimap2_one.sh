...
mkdir -p "$SCRATCH"
{
    ...
    df -h "$SCRATCH_BASE" | sed 's/^/scratch_df=/'
    echo "scratch_decision=node-local tmpfs /dev/shm selected when available; task explicitly permits /dev/shm and this avoids streaming large intermediate PAF output to moosefs"
    echo "min_runtime_before_unflushed_cancel=$MIN_RUNTIME_BEFORE_UNFLUSHED_CANCEL"
    echo "runtime_policy=do not cancel solely because the node-local gzip is header-sized or unflushed before this runtime; cancel only for hard failure, OOM, devshm exhaustion, or manual/user decision"
    echo "target=$TARGET"
    echo "query=$QUERY"
