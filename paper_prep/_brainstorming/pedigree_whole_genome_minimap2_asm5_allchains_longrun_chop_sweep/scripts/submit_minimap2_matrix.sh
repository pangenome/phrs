...
TIME="${MINIMAP2_TIME:-24:00:00}"
MIN_RUNTIME_BEFORE_UNFLUSHED_CANCEL="${MINIMAP2_MIN_RUNTIME_BEFORE_UNFLUSHED_CANCEL:-08:00:00}"

printf "run_label\tcomparison_id\tparameter_set\tjob_id\tstatus\tstdout\tstderr\toutput_paf\toutput_size_bytes\tsha256\tminimap2_options\tscratch_base\tminimap2_bin\trequested_time\trequested_mem\trequested_cpus\tmin_runtime_before_unflushed_cancel\truntime_policy\n" > "$STATUS"
...
tail -n +2 "$COMPARISONS" | cut -f1 | while read -r cid; do
    ...
    tail -n +2 "$PARAMETERS" | cut -f1,3 | while IFS=$'\t' read -r parameter_set opts; do
        ...
        submit_out="$(sbatch --parsable \
            --export=ALL,PACKAGE_DIR="$PACKAGE_DIR",MINIMAP2_SCRATCH_BASE="$SCRATCH_BASE",MINIMAP2_BIN="$MINIMAP2_BIN",MINIMAP2_RUN_LABEL="$MINIMAP2_RUN_LABEL",MINIMAP2_SUFFIX="$MINIMAP2_SUFFIX",MINIMAP2_MIN_RUNTIME_BEFORE_UNFLUSHED_CANCEL="$MIN_RUNTIME_BEFORE_UNFLUSHED_CANCEL" \
        ...
        printf "%s\t%s\t%s\t%s\tSUBMITTED\t%s\t%s\t%s\t\t\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" \
            "$MINIMAP2_RUN_LABEL" "$cid" "$parameter_set" "$job_id" "$stdout_actual" "$stderr_actual" "$out_paf" "$opts" "$SCRATCH_BASE" "$MINIMAP2_BIN" "$TIME" "$MEM" "$CPUS" "$MIN_RUNTIME_BEFORE_UNFLUSHED_CANCEL" "do_not_cancel_for_header_only_or_unflushed_output_before_min_runtime" >> "$STATUS"
    done
done
