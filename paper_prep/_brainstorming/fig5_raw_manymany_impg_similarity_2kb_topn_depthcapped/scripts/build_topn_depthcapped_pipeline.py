...
def job_script(path: Path, task_manifest: Path, top_n: int, max_candidates: int, time_limit: str, cpus: int, max_parallel: int) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    lines = [
        "#!/usr/bin/env bash",
...
def main() -> None:
    ...
    for key, pair_rows in sorted(defaultdict(list, {k: [r for r in new_rows if (r["method"], r["comparison_id"]) == k] for k in {(r["method"], r["comparison_id"]) for r in new_rows}}).items()):
        ...
        job_script(script, task_manifest, args.top_n, args.max_candidates, args.time, args.cpus, args.max_parallel)
        job_id = ""
        state = "CREATED"
        if args.submit:
    ...
    metadata = {"created_utc": utc_now(), "top_n": args.top_n, "max_candidates": args.max_candidates, "centromere_pad": args.centromere_pad, "source": str(SOURCE), "submitted": args.submit}
