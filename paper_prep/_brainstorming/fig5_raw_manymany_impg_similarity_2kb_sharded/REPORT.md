...
## Deferred Finalization Check: 2026-06-29T00:54:47Z

Finalization was re-checked from WG task `finalize-fig5-raw-17` in worktree
`/moosefs/erikg/phrs/.wg-worktrees/agent-2892`. The live shard tree remains:
...
present under the live shard tree, apart from the committed finalizer script and
Python bytecode cache. The harvested Slurm wrapper in
...
The Slurm guardrail still blocks assembly because arrays `1706840`-`1706845`
remain active. Exact `squeue` state at `2026-06-29T00:54:47Z`:
...
1706844_[0-148%6]|PENDING|0:00|(Priority)|fig5_impg_wfmash_PAN027pat_vs_P
1706845_[0-151%6]|PENDING|0:00|(Priority)|fig5_impg_wfmash_PAN028mat_vs_P
1706840_[85-151%6]|PENDING|0:00|(Resources)|fig5_impg_sweepg_PAN027mat_vs_P
1706861|PENDING|0:00|(Dependency)|fig5_impg_finalize_2kb
1706840_84|RUNNING|14:21|octopus11|fig5_impg_sweepg_PAN027mat_vs_P
1706840_78|RUNNING|18:59:09|octopus09|fig5_impg_sweepg_PAN027mat_vs_P
```

`sacct` additionally reported `1706840_78` as `RUNNING` on `octopus09` since
`2026-06-28T05:55:27`, `1706840_79` as `COMPLETED` on `octopus11` with exit
code `0:0` at `2026-06-29T00:33:37`, `1706840_80` through `1706840_83` as
`COMPLETED`, `1706840_84` as `RUNNING` on `octopus11` since
`2026-06-29T00:40:15`, `1706840_[85-151%6]` pending, and arrays `1706841`,
`1706842`, `1706843`, `1706844`, and `1706845` pending with no node assigned.
The concrete live log paths for the currently running shards are:
...
- `/moosefs/erikg/phrs/.wg-worktrees/agent-2837/paper_prep/_brainstorming/fig5_raw_manymany_impg_similarity_2kb_sharded/logs/sweepga_fastga_frequency32.PAN027mat_vs_PAN010_joint.shard_84.1706840.out`
- `/moosefs/erikg/phrs/.wg-worktrees/agent-2837/paper_prep/_brainstorming/fig5_raw_manymany_impg_similarity_2kb_sharded/logs/sweepga_fastga_frequency32.PAN027mat_vs_PAN010_joint.shard_84.1706840.err`
...
finalizer or harvest has updated the manifest yet. The checked-in, live, and
main-target `manifests/assembled_outputs.tsv` files still have only their
headers. Neither the live tree nor the main target tree currently has
`outputs/assembled/` or `summaries/` products, so the six assembled compressed
outputs and downstream summary tables remain correctly blocked by
RUNNING/PENDING Slurm state.

No WFMASH, SweepGA/FastGA, minimap2, seqwish, odgi, alignment, shard
normalization, finalizer, rsync harvest, or partial assembly command was run.
Incomplete shards were not marked as data failures. A delayed WG follow-up was
created to re-check Slurm state: `finalize-fig5-raw-18`. Because a direct child
task would risk exceeding the configured graph depth limit, that task was
created at the same graph level as this one instead of as a direct child.
...
## Latest Blocked-State Pointer: 2026-06-29T00:54:47Z

The latest re-check is the `finalize-fig5-raw-17` section above. It records
finalizer `1706861` still `PENDING` on `Dependency`, `1706840_78` and
`1706840_84` still `RUNNING`, `1706840_[85-151%6]` still `PENDING`, and arrays
...
follow-up `finalize-fig5-raw-18` was created to re-check the same dependency
chain.

## Deferred Finalization Check: 2026-06-29T06:19:39Z

Finalization was re-checked from WG task `finalize-fig5-raw-22` in worktree
`/moosefs/erikg/phrs/.wg-worktrees/agent-2897`. The live shard tree remains:
...
`*1706861*`, `*final*`, or `slurm-1706861*` found only
`scripts/finalize_2kb_sharded_impg.py` and cached bytecode in the live tree;
no `logs/finalize_after_arrays.*` or `slurm-1706861*` file is present.
Therefore job `1706861` has not yet owned tmp shard normalization, finalizer
execution, or rsync harvest.

The Slurm guardrail still blocks assembly because arrays `1706840`-`1706845`
remain active. Exact `squeue` state at this check:

```text
JOBID|STATE|REASON|TIME|TIME_LIMIT|NODES|NAME
1706841_[0-148%6]|PENDING|Priority|0:00|1-00:00:00|1|fig5_impg_sweepg_PAN027pat_vs_P
1706842_[0-151%6]|PENDING|Priority|0:00|1-00:00:00|1|fig5_impg_sweepg_PAN028mat_vs_P
1706843_[0-151%6]|PENDING|Priority|0:00|1-00:00:00|1|fig5_impg_wfmash_PAN027mat_vs_P
1706844_[0-148%6]|PENDING|Priority|0:00|1-00:00:00|1|fig5_impg_wfmash_PAN027pat_vs_P
1706845_[0-151%6]|PENDING|Priority|0:00|1-00:00:00|1|fig5_impg_wfmash_PAN028mat_vs_P
1706840_[99-151%6]|PENDING|Resources|0:00|1-00:00:00|1|fig5_impg_sweepg_PAN027mat_vs_P
1706861|PENDING|Dependency|0:00|4:00:00|1|fig5_impg_finalize_2kb
1706840_98|RUNNING|None|15:45|1-00:00:00|1|fig5_impg_sweepg_PAN027mat_vs_P
1706840_94|RUNNING|None|2:13:00|1-00:00:00|1|fig5_impg_sweepg_PAN027mat_vs_P
```

`sacct` additionally reported `1706840_78` as terminal `TIMEOUT`:

```text
1706840_78|fig5_impg_sweepg_PAN027mat_vs_P|workers|TIMEOUT|0:0|1-00:00:13|2026-06-28T05:55:27|2026-06-29T05:55:40
1706840_78.batch|batch||CANCELLED|0:15|1-00:00:14|2026-06-28T05:55:27|2026-06-29T05:55:41
```

The concrete live log paths for the timed-out shard are:

- `/moosefs/erikg/phrs/.wg-worktrees/agent-2837/paper_prep/_brainstorming/fig5_raw_manymany_impg_similarity_2kb_sharded/logs/sweepga_fastga_frequency32.PAN027mat_vs_PAN010_joint.shard_78.1706840.out`
- `/moosefs/erikg/phrs/.wg-worktrees/agent-2837/paper_prep/_brainstorming/fig5_raw_manymany_impg_similarity_2kb_sharded/logs/sweepga_fastga_frequency32.PAN027mat_vs_PAN010_joint.shard_78.1706840.err`

The tail of the shard 78 stderr ends with:

```text
slurmstepd: error: *** JOB 1706932 ON octopus09 CANCELLED AT 2026-06-29T05:55:47 DUE TO TIME LIMIT ***
```
...
- `/moosefs/erikg/phrs/.wg-worktrees/agent-2837/paper_prep/_brainstorming/fig5_raw_manymany_impg_similarity_2kb_sharded/logs/sweepga_fastga_frequency32.PAN027mat_vs_PAN010_joint.shard_98.1706840.out`
- `/moosefs/erikg/phrs/.wg-worktrees/agent-2837/paper_prep/_brainstorming/fig5_raw_manymany_impg_similarity_2kb_sharded/logs/sweepga_fastga_frequency32.PAN027mat_vs_PAN010_joint.shard_98.1706840.err`

The checked-in and live `manifests/shard_completion_manifest.tsv` files still
have 906 data rows plus header, and all 906 rows remain
`MISSING_OR_INCOMPLETE`. The checked-in and live
`manifests/assembled_outputs.tsv` files still have only their header. The live
tree currently has shard tmp outputs only for the first array; shard `0078` is
a partial timed-out tmp file, shards `0094` and `0098` are current tmp files,
and neither the live tree nor the checked-in mirror currently has
`outputs/assembled/` or `summaries/` products. Therefore the six assembled
compressed outputs and downstream summary tables remain correctly blocked by
RUNNING/PENDING Slurm state plus a terminal timeout that must be handled by the
Slurm pipeline owner before any finalization can be trusted.

No WFMASH, SweepGA/FastGA, minimap2, seqwish, odgi, alignment, shard
normalization, finalizer, rsync harvest, or partial assembly command was run.
Incomplete shards were not marked as data failures. A delayed WG follow-up was
created to re-check Slurm state: `finalize-fig5-raw-23`. Because a direct child
task would exceed the configured graph depth limit, that task was created at
the same graph level as this one with `finalize-fig5-raw-11` as its upstream
dependency instead of as a direct child.

That follow-up should again inspect dependency finalizer job `1706861` before
manual finalization. If any array remains running or pending, it should record
the exact active state and defer again. If arrays are terminal but unsuccessful,
it should diagnose failed Slurm shards with concrete log paths and avoid
promoting incomplete tmp output. Only after all six arrays are terminal and
successful should it normalize tmp shard filenames if needed, harvest or run
the finalizer, preserve all-hit assembled outputs under `outputs/assembled/`,
and verify that the plotting summaries reduce to one best hit per 2 kb query
window using the documented deterministic tie-break: highest
similarity/ANI/support score, then aligned/support length, then lexical target
coordinates. This task does not supersede the failed `finalize-fig5-raw`; it
preserves the guardrail and defers finalization until the Slurm dependency
chain reaches a terminal successful state.

## Latest Blocked-State Pointer: 2026-06-29T06:19:39Z

The latest re-check is the `finalize-fig5-raw-22` section above. It records
finalizer `1706861` still `PENDING` on `Dependency`, `1706840_94` and
`1706840_98` still `RUNNING`, `1706840_[99-151%6]` still `PENDING`, arrays
`1706841`-`1706845` still `PENDING`, and `1706840_78` terminal `TIMEOUT` with
concrete live logs. Finalization, shard normalization, rsync harvest, and any
alignment/graph-build command remain blocked. Delayed follow-up
`finalize-fig5-raw-23` was created to re-check the same dependency chain.

## Deferred Finalization Check: 2026-06-29T01:59:23Z

Finalization was re-checked from WG task `finalize-fig5-raw-18` in worktree
`/moosefs/erikg/phrs/.wg-worktrees/agent-2893`. The live shard tree remains:
...
No live or main-tree log file matching `*1706861*`, `*final*`, or
`slurm-1706861*` was present under the shard-tree `logs/` directories. The
harvested Slurm wrapper in `jobs/finalize_after_slurm_arrays.slurm.sh` would
write `logs/finalize_after_arrays.%j.out` and
`logs/finalize_after_arrays.%j.err` in the main target tree, but those logs are
absent because job `1706861` is still pending on its dependency and has not
owned tmp shard normalization, finalizer execution, or rsync harvest.

The Slurm guardrail still blocks assembly because arrays `1706840`-`1706845`
remain active. Exact `squeue` state at `2026-06-29T01:59:23Z`:
...
1706844_[0-148%6] workers   fig5_impg_wfmash_PAN027pat_vs_P           PENDING      0:00     1-00:00:00 1     (Priority)
1706845_[0-151%6] workers   fig5_impg_wfmash_PAN028mat_vs_P           PENDING      0:00     1-00:00:00 1     (Priority)
1706840_[91-151%6] workers  fig5_impg_sweepg_PAN027mat_vs_P           PENDING      0:00     1-00:00:00 1     (Resources)
1706861           workers   fig5_impg_finalize_2kb                    PENDING      0:00     4:00:00    1     (Dependency)
1706840_90        workers   fig5_impg_sweepg_PAN027mat_vs_P           RUNNING      14:15    1-00:00:00 1     octopus11
1706840_78        workers   fig5_impg_sweepg_PAN027mat_vs_P           RUNNING      20:03:56 1-00:00:00 1     octopus09
```

`sacct` additionally reported `1706840_78` as `RUNNING` on `octopus09` since
`2026-06-28T05:55:27`, `1706840_90` as `RUNNING` on `octopus11` since
`2026-06-29T01:45:08`, `1706840_[91-151%6]` pending, and arrays `1706841`,
`1706842`, `1706843`, `1706844`, and `1706845` pending with no node assigned.
The concrete live log paths for the two running shards are:

- `/moosefs/erikg/phrs/.wg-worktrees/agent-2837/paper_prep/_brainstorming/fig5_raw_manymany_impg_similarity_2kb_sharded/logs/sweepga_fastga_frequency32.PAN027mat_vs_PAN010_joint.shard_78.1706840.out`
- `/moosefs/erikg/phrs/.wg-worktrees/agent-2837/paper_prep/_brainstorming/fig5_raw_manymany_impg_similarity_2kb_sharded/logs/sweepga_fastga_frequency32.PAN027mat_vs_PAN010_joint.shard_78.1706840.err`
- `/moosefs/erikg/phrs/.wg-worktrees/agent-2837/paper_prep/_brainstorming/fig5_raw_manymany_impg_similarity_2kb_sharded/logs/sweepga_fastga_frequency32.PAN027mat_vs_PAN010_joint.shard_90.1706840.out`
- `/moosefs/erikg/phrs/.wg-worktrees/agent-2837/paper_prep/_brainstorming/fig5_raw_manymany_impg_similarity_2kb_sharded/logs/sweepga_fastga_frequency32.PAN027mat_vs_PAN010_joint.shard_90.1706840.err`

Both the live and checked-in `manifests/shard_completion_manifest.tsv` files
have 906 data rows plus header. The live manifest still has 906
`MISSING_OR_INCOMPLETE` rows because the Slurm run has not completed and no
finalizer or harvest has updated the manifest yet. The checked-in and live
`manifests/assembled_outputs.tsv` files still have only their headers. Neither
the live tree nor the checked-in mirror currently has `outputs/assembled/` or
...
normalization, finalizer, rsync harvest, or partial assembly command was run.
Incomplete shards were not marked as data failures. A delayed WG follow-up was
created to re-check Slurm state: `finalize-fig5-raw-19`. Because a direct child
task would exceed the configured graph depth limit, that task was created at
the same graph level as this one with `finalize-fig5-raw-11` as its upstream
dependency instead of as a direct child.
...
## Latest Blocked-State Pointer: 2026-06-29T01:59:23Z

The latest re-check is the `finalize-fig5-raw-18` section above. It records
finalizer `1706861` still `PENDING` on `Dependency`, `1706840_78` and
`1706840_90` still `RUNNING`, `1706840_[91-151%6]` still `PENDING`, and arrays
`1706841`-`1706845` still `PENDING`. Finalization, shard normalization, rsync
harvest, and any alignment/graph-build command remain blocked. Delayed
follow-up `finalize-fig5-raw-19` was created to re-check the same dependency
chain.

## Deferred Finalization Check: 2026-06-29T03:05:37Z

Finalization was re-checked from WG task `finalize-fig5-raw-19` in worktree
`/moosefs/erikg/phrs/.wg-worktrees/agent-2894`. The live shard tree remains:
...
No live or main-tree finalizer log exists yet because job `1706861` is still
pending on its dependency and has not owned tmp shard normalization, finalizer
execution, or rsync harvest.

The Slurm guardrail still blocks assembly because arrays `1706840`-`1706845`
remain active. Exact `squeue` state at `2026-06-29T03:05:37Z`:
...
1706844_[0-148%6]|PENDING|0:00|1|(Priority)|fig5_impg_wfmash_PAN027pat_vs_P
1706845_[0-151%6]|PENDING|0:00|1|(Priority)|fig5_impg_wfmash_PAN028mat_vs_P
1706840_[91-151%6]|PENDING|0:00|1|(Resources)|fig5_impg_sweepg_PAN027mat_vs_P
1706861|PENDING|0:00|1|(Dependency)|fig5_impg_finalize_2kb
1706840_90|RUNNING|1:19:08|1|octopus11|fig5_impg_sweepg_PAN027mat_vs_P
1706840_78|RUNNING|21:08:49|1|octopus09|fig5_impg_sweepg_PAN027mat_vs_P
```
...
checked-in `manifests/assembled_outputs.tsv` still has only its header. Neither
the live tree nor the checked-in mirror currently has `outputs/assembled/` or
`summaries/` products, so the six assembled compressed outputs and downstream
summary tables remain correctly blocked by RUNNING/PENDING Slurm state.
...
normalization, finalizer, rsync harvest, or partial assembly command was run.
Incomplete shards were not marked as data failures. A delayed WG follow-up was
created to re-check Slurm state: `finalize-fig5-raw-20`. Because a direct child
task would exceed the configured graph depth limit, that task was created at
the same graph level as this one with `finalize-fig5-raw-11` as its upstream
dependency instead of as a direct child.
...
## Latest Blocked-State Pointer: 2026-06-29T03:05:37Z

The latest re-check is the `finalize-fig5-raw-19` section above. It records
finalizer `1706861` still `PENDING` on `Dependency`, `1706840_78` and
`1706840_90` still `RUNNING`, `1706840_[91-151%6]` still `PENDING`, and arrays
`1706841`-`1706845` still `PENDING`. Finalization, shard normalization, rsync
harvest, and any alignment/graph-build command remain blocked. Delayed
follow-up `finalize-fig5-raw-20` was created to re-check the same dependency
chain.

## Deferred Finalization Check: 2026-06-29T04:09:19Z

Finalization was re-checked from WG task `finalize-fig5-raw-20` in worktree
`/moosefs/erikg/phrs/.wg-worktrees/agent-2895`. The live shard tree remains:
...
```text
JobID|JobName|Partition|State|ExitCode|Elapsed|Start|End|NodeList
1706861|fig5_impg_finalize_2kb|workers|PENDING|0:0|00:00:00|Unknown|Unknown|None assigned
```

No live or main-tree finalizer log exists yet. Searches for files matching
`*1706861*`, `*final*`, or `slurm-1706861*` found only the finalizer scripts
and cached bytecode, plus the submitted wrapper
...
The Slurm guardrail still blocks assembly because arrays `1706840`-`1706845`
remain active. Exact `squeue` state at `2026-06-29T04:09:19Z`:

```text
JOBID              PARTITION NAME                                      USER  ST START_TIME           TIME     TIME_LIMIT NODES NODELIST(REASON)
1706841_[0-148%6] workers   fig5_impg_sweepg_PAN027pat_vs_            erikg PD 2027-06-26T11:26:06  0:00     1-00:00:00 1     (Priority)
1706842_[0-151%6] workers   fig5_impg_sweepg_PAN028mat_vs_            erikg PD 2027-06-26T11:26:06  0:00     1-00:00:00 1     (Priority)
1706843_[0-151%6] workers   fig5_impg_wfmash_PAN027mat_vs_            erikg PD 2027-06-26T11:26:06  0:00     1-00:00:00 1     (Priority)
1706844_[0-148%6] workers   fig5_impg_wfmash_PAN027pat_vs_            erikg PD 2027-06-26T11:26:06  0:00     1-00:00:00 1     (Priority)
1706845_[0-151%6] workers   fig5_impg_wfmash_PAN028mat_vs_            erikg PD 2027-06-26T11:26:06  0:00     1-00:00:00 1     (Priority)
1706840_[95-151%6] workers  fig5_impg_sweepg_PAN027mat_vs_            erikg PD 2026-06-29T05:55:27  0:00     1-00:00:00 1     (Resources)
1706861           workers   fig5_impg_finalize_2kb                    erikg PD N/A                  0:00     4:00:00    1     (Dependency)
1706840_94        workers   fig5_impg_sweepg_PAN027mat_vs_            erikg R  2026-06-29T04:03:54  5:10     1-00:00:00 1     octopus11
1706840_78        workers   fig5_impg_sweepg_PAN027mat_vs_            erikg R  2026-06-28T05:55:27  22:13:37 1-00:00:00 1     octopus09
```
...
normalization, finalizer, rsync harvest, or partial assembly command was run.
Incomplete shards were not marked as data failures. A delayed WG follow-up was
created to re-check Slurm state: `finalize-fig5-raw-21`. Because a direct child
task would exceed the configured graph depth limit, that task was created at
the same graph level as this one with `finalize-fig5-raw-11` as its upstream
dependency instead of as a direct child.
...
## Latest Blocked-State Pointer: 2026-06-29T04:09:19Z

The latest re-check is the `finalize-fig5-raw-20` section above. It records
finalizer `1706861` still `PENDING` on `Dependency`, `1706840_78` and
...
The latest re-check is the `finalize-fig5-raw-22` section in this report. It
records finalizer `1706861` still `PENDING` on `Dependency`, `1706840_94` and
`1706840_98` still `RUNNING`, `1706840_[99-151%6]` still `PENDING`, arrays
`1706841`-`1706845` still `PENDING`, and `1706840_78` terminal `TIMEOUT` with
concrete live logs. Finalization, shard normalization, rsync harvest, and any
alignment/graph-build command remain blocked. Delayed follow-up
`finalize-fig5-raw-23` was created to re-check the same dependency chain. This
task does not supersede the failed `finalize-fig5-raw`; it preserves the Slurm
guardrail and defers finalization until the dependency chain reaches a
terminal successful state.

## Deferred Finalization Check: 2026-06-29T07:22:42Z

Finalization was re-checked from WG task `finalize-fig5-raw-23` in worktree
`/moosefs/erikg/phrs/.wg-worktrees/agent-2898`. The live shard tree remains:
...
1706861|fig5_impg_finalize_2kb|workers|PENDING|0:0|00:00:00|Unknown|Unknown
```

No live or main-tree finalizer log exists yet. Searches for files matching
`*1706861*`, `*final*`, or `slurm-1706861*` found only finalizer scripts,
cached bytecode, and the submitted wrapper
`jobs/finalize_after_slurm_arrays.slurm.sh`; no `logs/finalize_after_arrays.*`
file is present. Therefore job `1706861` has not yet owned tmp shard
normalization, finalizer execution, or rsync harvest.
...
The Slurm guardrail still blocks assembly because arrays `1706840`-`1706845`
remain active. Exact `squeue` state at `2026-06-29T07:22:42Z`:

```text
JOBID              PARTITION NAME                                      USER  ST TIME     TIME_LIMIT NODES NODELIST(REASON)
1706841_[0-148%6] workers   fig5_impg_sweepg_PAN027pat_vs_P           erikg PD 0:00     7-00:00:00 1     (Priority)
1706842_[0-151%6] workers   fig5_impg_sweepg_PAN028mat_vs_P           erikg PD 0:00     7-00:00:00 1     (Priority)
1706843_[0-151%6] workers   fig5_impg_wfmash_PAN027mat_vs_P           erikg PD 0:00     7-00:00:00 1     (Priority)
1706844_[0-148%6] workers   fig5_impg_wfmash_PAN027pat_vs_P           erikg PD 0:00     7-00:00:00 1     (Priority)
1706845_[0-151%6] workers   fig5_impg_wfmash_PAN028mat_vs_P           erikg PD 0:00     7-00:00:00 1     (Priority)
1706840_[119-151%6] workers fig5_impg_sweepg_PAN027mat_vs_P           erikg PD 0:00     7-00:00:00 1     (Resources)
1706861           workers   fig5_impg_finalize_2kb                    erikg PD 0:00     4:00:00    1     (Dependency)
1706840_118       workers   fig5_impg_sweepg_PAN027mat_vs_P           erikg R  14:52    1-00:00:00 1     octopus09
1706840_115       workers   fig5_impg_sweepg_PAN027mat_vs_P           erikg R  18:53    1-00:00:00 1     octopus11
```

`sacct` additionally reported `1706840_115` as `RUNNING` on `octopus11` since
`2026-06-29T07:03:17`, `1706840_118` as `RUNNING` on `octopus09` since
`2026-06-29T07:07:18`, `1706840_[119-151%6]` pending, and arrays `1706841`,
`1706842`, `1706843`, `1706844`, and `1706845` pending with no node assigned.
It also preserves the earlier failed-shard signal from the prior drain:
`1706840_78` is terminal `TIMEOUT` with batch step `CANCELLED`, started
`2026-06-28T05:55:27` and ended `2026-06-29T05:55:40`. Because the overall
array chain remains RUNNING/PENDING, finalization is blocked rather than
marking an incomplete shard output as valid.

Concrete live log paths for the currently running shards are:

- `/moosefs/erikg/phrs/.wg-worktrees/agent-2837/paper_prep/_brainstorming/fig5_raw_manymany_impg_similarity_2kb_sharded/logs/sweepga_fastga_frequency32.PAN027mat_vs_PAN010_joint.shard_115.1706840.out`
- `/moosefs/erikg/phrs/.wg-worktrees/agent-2837/paper_prep/_brainstorming/fig5_raw_manymany_impg_similarity_2kb_sharded/logs/sweepga_fastga_frequency32.PAN027mat_vs_PAN010_joint.shard_115.1706840.err`
- `/moosefs/erikg/phrs/.wg-worktrees/agent-2837/paper_prep/_brainstorming/fig5_raw_manymany_impg_similarity_2kb_sharded/logs/sweepga_fastga_frequency32.PAN027mat_vs_PAN010_joint.shard_118.1706840.out`
- `/moosefs/erikg/phrs/.wg-worktrees/agent-2837/paper_prep/_brainstorming/fig5_raw_manymany_impg_similarity_2kb_sharded/logs/sweepga_fastga_frequency32.PAN027mat_vs_PAN010_joint.shard_118.1706840.err`

The checked-in `manifests/shard_completion_manifest.tsv` still has 906 data
rows plus header, and all 906 rows remain `MISSING_OR_INCOMPLETE`. The live
tree currently has shard tmp outputs only for the first array; shard `0115`
and shard `0118` are still tmp files without a final `.gz` suffix because
their Slurm tasks remain running. Neither the live tree nor the checked-in
mirror currently has `outputs/assembled/` or `summaries/` products, so the six
assembled compressed outputs and downstream summary tables remain correctly
blocked by RUNNING/PENDING Slurm state.

No WFMASH, SweepGA/FastGA, minimap2, seqwish, odgi, alignment, shard
normalization, finalizer, rsync harvest, or partial assembly command was run.
Incomplete shards were not marked as data failures. A delayed WG follow-up was
created to re-check Slurm state: `finalize-fig5-raw-24`. Because a direct child
task would exceed the configured graph depth limit, that task was created at
the same graph level as this one with `finalize-fig5-raw-11` as its upstream
dependency instead of as a direct child.
...
That follow-up should again inspect dependency finalizer job `1706861` before
manual finalization. Only after all six arrays are terminal and successful
should it normalize tmp shard filenames if needed, harvest or run the
finalizer, preserve all-hit assembled outputs under `outputs/assembled/`, and
verify that the plotting summaries reduce to one best hit per 2 kb query
window using the documented deterministic tie-break: highest
similarity/ANI/support score, then aligned/support length, then lexical target
coordinates. This task does not supersede the failed `finalize-fig5-raw`; it
preserves the guardrail and defers finalization until the Slurm dependency
...
## Latest Blocked-State Pointer: 2026-06-29T07:22:42Z

The latest re-check is the `finalize-fig5-raw-23` section above. It records
finalizer `1706861` still `PENDING` on `Dependency`, `1706840_115` and
`1706840_118` still `RUNNING`, `1706840_[119-151%6]` still `PENDING`, arrays
`1706841`-`1706845` still `PENDING`, and `1706840_78` terminal `TIMEOUT` from
the prior drain. Finalization, shard normalization, rsync harvest, and any
alignment/graph-build command remain blocked. Delayed follow-up
`finalize-fig5-raw-24` was created to re-check the same dependency chain. This
task does not supersede the failed `finalize-fig5-raw`; it preserves the Slurm
guardrail and defers finalization until the dependency chain reaches a
terminal successful state.

## Deferred Finalization Check: 2026-06-29T08:28:48Z

Finalization was re-checked from WG task `finalize-fig5-raw-24` in worktree
`/moosefs/erikg/phrs/.wg-worktrees/agent-2899`. The live shard tree remains:
...
No live finalizer log exists yet. Searching the live tree for files matching
`*1706861*` or `*final*` found only `scripts/finalize_2kb_sharded_impg.py` and
its bytecode cache. Therefore job `1706861` has not yet owned tmp shard
normalization, finalizer execution, or rsync harvest.

The Slurm guardrail still blocks assembly because arrays `1706840`-`1706845`
remain active. Exact `squeue` state at this re-check:
...
1706844_[0-148%6]|PENDING|0:00|1|(Priority)|fig5_impg_wfmash_PAN027pat_vs_P
1706845_[0-151%6]|PENDING|0:00|1|(Priority)|fig5_impg_wfmash_PAN028mat_vs_P
1706840_[134-151%6]|PENDING|0:00|1|(Resources)|fig5_impg_sweepg_PAN027mat_vs_P
1706861|PENDING|0:00|1|(Dependency)|fig5_impg_finalize_2kb
1706840_133|RUNNING|12:22|1|octopus08|fig5_impg_sweepg_PAN027mat_vs_P
1706840_132|RUNNING|13:01|1|octopus07|fig5_impg_sweepg_PAN027mat_vs_P
1706840_130|RUNNING|14:11|1|octopus09|fig5_impg_sweepg_PAN027mat_vs_P
1706840_127|RUNNING|15:36|1|octopus10|fig5_impg_sweepg_PAN027mat_vs_P
1706840_115|RUNNING|1:24:33|1|octopus11|fig5_impg_sweepg_PAN027mat_vs_P
```

`sacct` additionally reported:

- `1706840_115` `RUNNING`, started `2026-06-29T07:03:17`, end `Unknown`.
- `1706840_127` `RUNNING`, started `2026-06-29T08:12:14`, end `Unknown`.
- `1706840_130` `RUNNING`, started `2026-06-29T08:13:39`, end `Unknown`.
- `1706840_132` `RUNNING`, started `2026-06-29T08:14:49`, end `Unknown`.
- `1706840_133` `RUNNING`, started `2026-06-29T08:15:28`, end `Unknown`.
- `1706840_[134-151%6]` `PENDING`.
- Arrays `1706841_[0-148%6]`, `1706842_[0-151%6]`,
  `1706843_[0-151%6]`, `1706844_[0-148%6]`, and
  `1706845_[0-151%6]` `PENDING`.

The earlier failed-shard signal remains present but is not actionable while the
overall dependency chain is still active: `1706840_78` is terminal `TIMEOUT`
with batch step `CANCELLED`, started `2026-06-28T05:55:27` and ended
`2026-06-29T05:55:40`.
...
- `/moosefs/erikg/phrs/.wg-worktrees/agent-2837/paper_prep/_brainstorming/fig5_raw_manymany_impg_similarity_2kb_sharded/logs/sweepga_fastga_frequency32.PAN027mat_vs_PAN010_joint.shard_115.1706840.out`
- `/moosefs/erikg/phrs/.wg-worktrees/agent-2837/paper_prep/_brainstorming/fig5_raw_manymany_impg_similarity_2kb_sharded/logs/sweepga_fastga_frequency32.PAN027mat_vs_PAN010_joint.shard_115.1706840.err`
- `/moosefs/erikg/phrs/.wg-worktrees/agent-2837/paper_prep/_brainstorming/fig5_raw_manymany_impg_similarity_2kb_sharded/logs/sweepga_fastga_frequency32.PAN027mat_vs_PAN010_joint.shard_127.1706840.out`
- `/moosefs/erikg/phrs/.wg-worktrees/agent-2837/paper_prep/_brainstorming/fig5_raw_manymany_impg_similarity_2kb_sharded/logs/sweepga_fastga_frequency32.PAN027mat_vs_PAN010_joint.shard_127.1706840.err`
- `/moosefs/erikg/phrs/.wg-worktrees/agent-2837/paper_prep/_brainstorming/fig5_raw_manymany_impg_similarity_2kb_sharded/logs/sweepga_fastga_frequency32.PAN027mat_vs_PAN010_joint.shard_130.1706840.out`
- `/moosefs/erikg/phrs/.wg-worktrees/agent-2837/paper_prep/_brainstorming/fig5_raw_manymany_impg_similarity_2kb_sharded/logs/sweepga_fastga_frequency32.PAN027mat_vs_PAN010_joint.shard_130.1706840.err`
- `/moosefs/erikg/phrs/.wg-worktrees/agent-2837/paper_prep/_brainstorming/fig5_raw_manymany_impg_similarity_2kb_sharded/logs/sweepga_fastga_frequency32.PAN027mat_vs_PAN010_joint.shard_132.1706840.out`
- `/moosefs/erikg/phrs/.wg-worktrees/agent-2837/paper_prep/_brainstorming/fig5_raw_manymany_impg_similarity_2kb_sharded/logs/sweepga_fastga_frequency32.PAN027mat_vs_PAN010_joint.shard_132.1706840.err`
- `/moosefs/erikg/phrs/.wg-worktrees/agent-2837/paper_prep/_brainstorming/fig5_raw_manymany_impg_similarity_2kb_sharded/logs/sweepga_fastga_frequency32.PAN027mat_vs_PAN010_joint.shard_133.1706840.out`
- `/moosefs/erikg/phrs/.wg-worktrees/agent-2837/paper_prep/_brainstorming/fig5_raw_manymany_impg_similarity_2kb_sharded/logs/sweepga_fastga_frequency32.PAN027mat_vs_PAN010_joint.shard_133.1706840.err`

Concrete live log paths for the prior terminal timeout are:
...
The checked-in `manifests/shard_completion_manifest.tsv` still has 906 data
rows plus header, and all 906 rows remain `MISSING_OR_INCOMPLETE`. The live
tree has shard tmp outputs only for the first array so far; shards `0115`,
`0127`, `0130`, `0132`, and `0133` are still uncompressed tmp files tied to
running Slurm tasks, while `0078` remains an uncompressed tmp file tied to the
earlier timeout. Neither the live tree nor the checked-in mirror currently has
`outputs/assembled/` or `summaries/` products, so the six assembled compressed
outputs and downstream summary tables remain correctly blocked by
RUNNING/PENDING Slurm state.
...
normalization, finalizer, rsync harvest, or partial assembly command was run.
Incomplete shards were not marked as data failures. A delayed WG follow-up was
created to re-check Slurm state: `finalize-fig5-raw-25`. Because a direct
child task would exceed the configured graph depth limit, that task was
created at the same graph level as this one with `finalize-fig5-raw-11` as its
upstream dependency instead of as a direct child.
...
## Latest Blocked-State Pointer: 2026-06-29T08:28:48Z

The latest re-check is the `finalize-fig5-raw-24` section above. It records
finalizer `1706861` still `PENDING` on `Dependency`, `1706840_115`,
`1706840_127`, `1706840_130`, `1706840_132`, and `1706840_133` still
`RUNNING`, `1706840_[134-151%6]` still `PENDING`, arrays `1706841`-`1706845`
still `PENDING`, and `1706840_78` terminal `TIMEOUT` from the prior drain.
Finalization, shard normalization, rsync harvest, and any
alignment/graph-build command remain blocked. Delayed follow-up
`finalize-fig5-raw-25` was created to re-check the same dependency chain. This
task does not supersede the failed `finalize-fig5-raw`; it preserves the Slurm
guardrail and defers finalization until the Slurm dependency chain reaches a
terminal successful state.
