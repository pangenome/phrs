#!/usr/bin/env python3
"""Render the direct sweepGA 1:1 no-scaffold Fig5 schematic comparison.
...
read from event_manifest.tsv and selected_segments.sweepga_1to1.tsv in this
directory.
"""
...
SEGMENTS_TSV = HERE / "selected_segments.sweepga_1to1.tsv"

FULL_SVG = HERE / "fig5_synteny_recombination_sweepga_1to1_full.svg"
...
def draw_full_event(
    ...
    svg.text(
        TRACK_X0,
        bottom_y + 58,
        "Evidence blocks use local 0-based half-open offsets from direct sweepGA 1:1 no-scaffold PAFs; source labels show native 500 kb assembly windows.",
        11,
        "700",
        TEXT,
    )
...
def render(mode: str, output: Path) -> None:
    ...
    svg.text(42, 38, "Fig5 comparison: direct sweepGA 1:1 no-scaffold schematic", 24, "700")
    svg.text(
        42,
        61,
        "Direct sweepGA 1:1 no-scaffold comparison; coordinates are native 500 kb assembly-window offsets, not CHM13 projections.",
        13,
        "400",
        MUTED,
    )
...
def main() -> None:
    if not SEGMENTS_TSV.exists():
        build_script = HERE / "build_selected_segments_from_direct_paf.py"
