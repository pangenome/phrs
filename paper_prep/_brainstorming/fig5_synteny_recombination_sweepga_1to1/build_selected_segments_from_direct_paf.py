#!/usr/bin/env python3
"""Build Fig5 schematic geometry from direct sweepGA 1:1 no-scaffold PAFs.
...
Fig5 synteny schematic renderer. Query and target coordinates are native
assembly-window offsets parsed from PAF sequence names; no CHM13/reference
...
DIRECT_DIR = HERE.parent / "pedigree_direct_sweepga_concordance"
EVENTS_TSV = HERE / "event_manifest.tsv"
SOURCE_SEGMENTS_TSV = SOURCE_DIR / "selected_segments.tsv"
PAF_DIR = DIRECT_DIR / "filtered_paf"
OUT_TSV = HERE / "selected_segments.sweepga_1to1.tsv"
...
def comparison_ids(event: dict[str, str]) -> list[str]:
    if event["pair"] == "PAN027_vs_PAN011":
        return ["PAN027pat_vs_PAN011_hap1", "PAN027pat_vs_PAN011_hap2"]
    if event["pair"] == "PAN028_vs_PAN027":
        return ["PAN028mat_vs_PAN027_hap1", "PAN028mat_vs_PAN027_hap2"]
...
def make_row(
    ...
    return {
        ...
        "target_haplotype": f"{target.sample}#{target.haplotype}",
        ...
        "target_interval_recovery": "direct_sweepga_1to1_noscaffold_paf",
        ...
        "geometry_source": (
            "direct sweepGA filtered PAF; filter_id=one_one_noscaffold; "
...
def main() -> None:
    ...
    for event in events:
        ...
        for comparison_id in comparison_ids(event):
            paf_path = PAF_DIR / f"{comparison_id}.one_one_noscaffold.paf.gz"
            if not paf_path.exists():
                raise FileNotFoundError(paf_path)
            for fields in paf_rows(paf_path, query_name):
                output_rows.append(make_row(event, fields, paf_path, annotations))
    ...
    print(f"Wrote {len(output_rows)} direct sweepGA 1:1 no-scaffold segments to {rel(OUT_TSV)}")
