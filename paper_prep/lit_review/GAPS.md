...
## Process gap: 11 of 14 topical reviews never committed

Of the 14 topical lit reviews originally fanned out, three are committed: `topic_03` (pseudohomologous regions concept; agent-890 commit `2d98ec1`), `topic_04` (sex-chromosome pseudoautosomal regions; agent-906 commit `0082891`, merged after the first synthesis pass) and `topic_11` (pedigree-based recombination detection; agent-884 commit `78268c8`). The other 11 — `topic_01` (cytogenetic foundations), `topic_02` (TAR1 / TTAGGG / ITS), `topic_05` (acrocentric / rDNA / Robertsonian), `topic_06` (D4Z4 / DUX4 / FSHD), `topic_07` (concerted evolution / NAHR / gene conversion), `topic_08` (meiotic bouquet / nuclear-envelope tethering), `topic_09` (Hi-C / Pore-C / single-cell / meiotic 3D), `topic_10` (pangenome graphs / IMPG / transitive closure), `topic_12` (HPRC v1 / v2 population pangenomes), `topic_13` (subtelomere population genetics / Fst / out-of-Africa), `topic_14` (OR / OR4F gradient) — were marked done by their dispatched agents in 30–60 s without producing any committed output. Their evaluator scores ranged 0.02–0.17.

Consequence for this synthesis: Parts I (sex-chromosome subsection), II and III now draw on the topic_04 review at primary-paper depth; the rest of Parts I, III and IV draw their substrate from `paper_prep/synthesis/REFERENCES_v2.bib` (147 entries, augmented from the existing 24-entry `REFERENCES.bib` plus the topic_03, topic_04, topic_11 bibs), the end-to-end-report's literature ledger (`end-to-end-report/report/12_literature.md`), Andrea's appendix references (`13_appendix.md`), the `CROSSWALK.md` claim ↔ substrate map, and the lead-author clarifications recorded therein. The depth is competent at the chronological-narrative level but does not go to the per-paper case-by-case depth that `topic_03`, `topic_04` and `topic_11` do. A future pass redo of those 11 topics would substantially deepen Parts I–IV with primary-paper-by-primary-paper analysis.
...
- **Topic_04 PascalCase / `sexchrompars_*` deduplication.** The topic_04 review's `sexchrompars_charchar2003`, `sexchrompars_skaletsky2003`, `sexchrompars_ross2005` and `sexchrompars_mangs2007` keys reference the same papers as the existing `Charchar2003`, `Skaletsky2003`, `Ross2005` and `MangsMorris2007` PascalCase keys; only the PascalCase versions are kept in `REFERENCES_v2.bib`. `topic_04_sex_chromosome_pars.md` (read-only) continues to use `sexchrompars_*` keys and must therefore be compiled against `topic_04_sex_chromosome_pars.bib` (which has all 28 keys), not against `REFERENCES_v2.bib` alone. `SYNTHESIS.md` and `CHRONOLOGY.md` consistently use the PascalCase keys for these four papers.
