...
**R2 summary (paper only):** "Using 465 haplotype assemblies, this paper identifies 15,668 PHRs on 41 of 48 chromosome arms partitioned into 15 arm-level Leiden communities (100% sensitivity support) recovering all known cytogenetic groups. Sequence-similarity communities predict 3D nuclear proximity with Mantel rho = 0.66 across 14 independent assays (Hi-C, Pore-C, CiFi, Dip-C, sperm scHi-C, mouse meiotic Hi-C). Pedigree analysis of two independent families (WashU 538 patches at 92% within-community; CEPH1463 11 cross-assembler-validated features) catches the recombination events that perpetuate both sequence sharing and physical proximity, closing a four-link causal loop. Gene content, population structure (F_ST), single-genome generalization (RPE-1), and cross-species replication (mouse rho = 0.715) round out the picture."
