...
- **C3.** 466 near-complete haplotype assemblies from HPRC v2.
...
5. **466-haplotype headcount panel (C3).** Current ED1a/b cite 465. Either the abstract is wrong by one (CHM13 reference may or may not count toward the 466) or the dataset description needs to be updated. A 1-line headcount panel and an explicit Methods sentence would resolve this.
...
|---|---|---|---|---|
| `MANUSCRIPT_SKELETON.md` | Nature-format skeleton (~3000 words) built around "Population-scale subtelomeric communities mirror 3D nuclear organisation across human and mouse" thesis. 4 main figs + 8 ED. Headline numbers: 233 individuals, 465 hap, 18,827 flanks, 15,668 PHRs, 41 arms → 15 / 50 communities. Includes Pore-C, Dip-C, sperm, mouse meiotic, pedigree integration. | **partial** | Headline numbers (233 / 465 / 18,827 / 15,668), pipeline-stage list (§Headline numbers and §Methods), Fig 1a/1b/1c entries, Fig 3a (HG002 Pore-C contact matrix), and ED1, ED5, ED8 entries. | Skeleton is anchored on a wider thesis than the canonical abstract: it integrates Dip-C / sperm scHi-C / mouse meiotic / pedigree material that is **not** in ABSTRACT.md. Sections covering implicit-pangenome-graph methods (C2), NJ tree (C5), PCA (C6) are **absent** in the skeleton; sections covering pedigree / mouse / sperm scHi-C are **present but off-spec**. Needs a major prune + insertion of C2/C5/C6 sections. |
...
### C3. 466 near-complete haplotype assemblies from HPRC v2
- **Data:** `/moosefs/guarracino/HPRCv2/assemblies/` (HPRC v2 v1.1 assemblies, 233 individuals × 2 haplotypes = 465; CHM13 reference makes 466). Sample list: `/moosefs/guarracino/HPRCv2/PHR_III/HiC/sample_info.tsv`.
